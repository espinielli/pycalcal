(function() {
  ...
  function cahillKeyes(mg) {
    ...
    function preliminaries() {
      ...
      var m = 29, p = 15, p73a, lF, lT, lM, l, pointV, k = Math.sqrt(3);
      CK.lengthMA = 940 / 1e4 * CK.lengthMG;
      ...
      CK.sin60 = .866025403784439;
      CK.cos60 = .5;
      CK.yTranslate = CK.lengthMG * CK.sin60;
      CK.pointM = [ 0, 0 ];
      ...
      CK.pointE = [ pointN[0] - CK.lengthMA * Math.sin(30 * radians), pointN[1] - CK.lengthMA * Math.cos(30 * radians) ];
      CK.lengthGF = distance(CK.pointG, CK.pointF);
      CK.lengthBD = distance(CK.pointB, CK.pointD);
      CK.lengthBDE = CK.lengthBD + CK.lengthAB;
      ...
      pointU = [ CK.pointA[0] + CK.lengthAP73 * Math.cos(30 * radians), CK.pointA[1] + CK.lengthAP73 * Math.sin(30 * radians) ];
    ...
    function distance(p1, p2) {
      var deltaX = p1[0] - p2[0], deltaY = p1[1] - p2[1];
    ...
    function lineIntersection(point1, slope1, point2, slope2) {
      var m1 = Math.tan(slope1 * radians), m2 = Math.tan(slope2 * radians), p = [ 0, 0 ];
    ...
    function circleLineIntersection(cc, r, p1, p2) {
      var x1 = p1[0], y1 = p1[1], x2 = p2[0], y2 = p2[1], xc = cc[0], yc = cc[1], a = (x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1), b = 2 * ((x2 - x1) * (x1 - xc) + (y2 - y1) * (y1 - yc)), c = xc * xc + yc * yc + x1 * x1 + y1 * y1 - 2 * (xc * x1 + yc * y1) - r * r, d = b * b - 4 * a * c, u1 = 0, u2 = 0, x = 0, y = 0;
      if (a === 0) {
        return [ 0, [ 0, 0 ] ];
      } else if (d < 0) {
        return [ 0, [ 0, 0 ] ];
    ...
    function equator(m) {
      var l = CK.deltaMEq * m, jointE = [ 0, 0 ];
      if (l <= CK.lengthGF) {
    ...
    function parallel73(m) {
      var p73 = [ 0, 0 ], jF = jointF(m), lF = 0, xy = [ 0, 0 ];
      if (m <= 30) {
        p73[0] = CK.pointA[0] + CK.lengthAP73 * Math.cos(m * radians);
        p73[1] = CK.pointA[1] + CK.lengthAP73 * Math.sin(m * radians);
        lF = distance(jF, p73);
      } else {
    ...
    function parallel75(m) {
      return [ CK.pointA[0] + CK.lengthAP75 * Math.cos(m * radians), CK.pointA[1] + CK.lengthAP75 * Math.sin(m * radians) ];
    }
    function ll2mp(lon, lat) {
      var south = [ 0, 6, 7, 8, 5 ], o = truncate((lon + 180) / 90 + 1), p, m = lon + 180 - 90 * (o - 1) - 45, s = sgn(m);
      m = Math.abs(m);
      if (o === 5) o = 1;
      if (lat < 0) o = south[o];
      p = Math.abs(lat);
    ...
    function zoneC(m, p) {
      var l = 104 * (90 - p);
      return [ CK.pointA[0] + l * Math.cos(m * radians), CK.pointA[1] + l * Math.sin(m * radians) ];
    }
    function zoneD(m, p) {
      p = p;
      return equator(m);
    ...
    function zoneH(m, p) {
      var p75 = parallel75(45), p73a = parallel73(m), p73 = p73a.parallel73, lF = distance(CK.pointT, CK.pointB), lF75 = distance(CK.pointB, p75), l = (75 - p) * (lF75 + lF) / 2, xy = [ 0, 0 ];
      if (l <= lF75) {
    ...
    function zoneI(m, p) {
      var p73a = parallel73(m), lT = lengthTorridSegment(m), lM = lengthMiddleSegment(m), l = p * (lT + lM + p73a.lengthParallel73) / 73, xy;
      if (l <= lT) {
    ...
    function zoneJ(m, p) {
      var p75 = parallel75(m), lF75 = distance(jointF(m), p75), p73a = parallel73(m), p73 = p73a.parallel73, lF = p73a.lengthParallel73, l = (75 - p) * (lF75 - lF) / 2, xy = [ 0, 0 ];
      if (l <= lF75) {
    ...
    function zoneK(m, p, l15) {
      var l = p * l15 / 15, lT = lengthTorridSegment(m), lM = lengthMiddleSegment(m), xy = [ 0, 0 ];
      if (l <= lT) {
        xy = interpolate(l, lT, jointE(m), jointT(m));
      } else {
        l = l - lT;
    ...
    function zoneL(m, p, l15) {
      var p73a = parallel73(m), p73 = p73a.parallel73, lT = lengthTorridSegment(m), lM = lengthMiddleSegment(m), xy, lF = p73a.lengthParallel73, l = l15 + (p - 15) * (lT + lM + lF - l15) / 58;
      if (l <= lT) {
    ...
    function mp2xy(m, p) {
      var xy = [ 0, 0 ], lT, p15a, p15, flag15, l15;
      if (m === 0) {
        if (p >= 75) {
          xy = zoneA(m, p);
        } else {
    ...
    function forward(λ, φ) {
      var lon = λ * degrees, lat = φ * degrees, res = ll2mp(lon, lat), m = res[0], p = res[1], s = res[2], o = res[3], xy = mp2xy(m, p), mm = mj2g([ xy[0], s * xy[1] ], o);
      return mm;
    }
    return forward;
  }
  function cahillKeyesProjection() {
    var mg = 1e4, m = projectionMutator(cahillKeyes), p = m(mg);
    return p;
  }
  (d3.geo.cahillKeyes = cahillKeyesProjection).raw = cahillKeyes;
